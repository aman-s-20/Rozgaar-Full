...
const JobApplicant = require("../models/jobApplicantSchema");
...
async function updateJobInfo(req, res) {
    const user = req.user;
    if (user.type !== "recruiter") {
        res.status(401).json({
...
async function getUserPersonalDetail(req, res) {
...
async function applicationListForRecruiter(req, res) {
    const user = req.user;
    if (user.type !== "recruiter") {
        res.status(401).json({
...
async function getListofFinalApplicant(req, res) {
    const user = req.user;
    if (user.type === "recruiter") {
        let findParams = {
            recruiterId: user._id,
